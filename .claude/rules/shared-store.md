...
Value: {
    metadata: string   // OP から取得した Provider Metadata をそのまま JSON 文字列で保存
}
```

`metadata` フィールドには `/.well-known/openid-configuration` のレスポンス JSON をそのまま格納する。特定フィールドのみを抜き出さず全体を保存することで、`userinfo_endpoint` など将来利用するフィールドも再取得なしで参照できる。
