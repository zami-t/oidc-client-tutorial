...
- すべてのフィールドは非公開（小文字）にする
- 外部からのアクセスはメソッド経由のみとする
- コンストラクタ（`New〇〇`）経由でのみ生成する

これにより、外部から直接フィールドを書き換えられない不変性と、型安全性を保証する。

**Always Valid Domain Model**

モデルは「常に有効な状態」のみ存在できるよう設計する。
バリデーションが必要な場合は、コンストラクタで行い、不正な値を持つオブジェクトが生成されないことを保証する。

- バリデーションが必要な値はコンストラクタで検証し、失敗時は `error` を返す
- バリデーションが不要な値（外部から受け取りそのまま使うだけ、など）はエラーなしのコンストラクタでよい
- 「どこかで検証済みのはずの値」を後から検証するパターンは避ける
