...
ログの二重出力を防ぐため、各レイヤーでログを出す場面を明確に分ける。

| レイヤー | ログを出す場面 |
| -------- | -------------- |
| Presentation | インプット作成・バリデーション失敗のみ（usecase 呼び出し前） |
| Usecase | ユースケース内で発生するすべてのイベント（成功・失敗） |
| Infrastructure | ログしない |

実装パターンは `/presentation-layer`・`/usecase-layer` スキルのパターン例を参照。
